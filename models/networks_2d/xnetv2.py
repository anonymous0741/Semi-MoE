...
class CustomXNetv2(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        super(CustomXNetv2, self).__init__()
    ...
    def forward(self, x_main, x_L, x_H):
        ...
        M_d5 = self.M_Up5(M_x5)
        M_d5 = torch.cat((M_x4, M_d5), dim=1)
        M_d5 = self.M_Up_conv5(M_d5)

        M_d4 = self.M_Up4(M_d5)
        M_d4 = torch.cat((M_x3, M_d4), dim=1)
        M_d4 = self.M_Up_conv4(M_d4)

        M_d3 = self.M_Up3(M_d4)
        M_d3 = torch.cat((M_x2, M_d3), dim=1)
        M_d3 = self.M_Up_conv3(M_d3)

        M_d2 = self.M_Up2(M_d3)
        M_d2 = torch.cat((M_x1, M_d2), dim=1)
        M_d2 = self.M_Up_conv2(M_d2)
        ...
        # L decoder
        L_d5 = self.L_Up5(L_x5)
        L_d5 = torch.cat((L_x4, L_d5), dim=1)
        L_d5 = self.L_Up_conv5(L_d5)

        L_d4 = self.L_Up4(L_d5)
        L_d4 = torch.cat((L_x3, L_d4), dim=1)
        L_d4 = self.L_Up_conv4(L_d4)
        ...
        H_d3 = self.H_Up3(H_d4)
        H_d3 = torch.cat((H_x2, H_d3), dim=1)
        H_d3 = self.H_Up_conv3(H_d3)

        H_d2 = self.H_Up2(H_d3)
        H_d2 = torch.cat((H_x1, H_d2), dim=1)
        H_d2 = self.H_Up_conv2(H_d2)
...
class XNetv2_lm(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        super(XNetv2_lm, self).__init__()
...
class XNet_1b(nn.Module):
    def __init__(self, in_channels=3, num_classes=1):
        super(XNet_1b, self).__init__()
...
def custom_xnetv2(in_channels, num_classes):
    model = CustomXNetv2(in_channels, num_classes)
    init_weights(model, 'kaiming')
    return model
